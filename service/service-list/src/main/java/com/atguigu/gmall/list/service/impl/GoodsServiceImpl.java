...
public class GoodsServiceImpl implements GoodsService {
    ...
    /**
     * 增加商品的热度值
     *每次调用addScore方法时，都会为指定的商品ID增加热度分数。
     * 当热度分数达到500的倍数时，会更新商品的热度分数并保存到数据库（Elasticsearch）。
     * 这种机制可以帮助应用维护商品的热度排名，从而在用户搜索或浏览时提供更相关的结果。
     * @param goodsId
     */
    @Override
    public void addScore(Long goodsId) {
        ...
        if (!optionalGoods.isPresent()) {
            ...
            if (score.intValue() % 500 == 0) {
                ...
                // 保存   使用goodsDao的save方法保存更新后的商品对象。这会将商品的热度分数更新到Elasticsearch索引中。
                goodsDao.save(goods);
            }
        }
    }
