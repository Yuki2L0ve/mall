...
import com.example.mall.product.mapper.BaseAttrInfoMapper;
...
public class BaseAttrInfoServiceImpl implements BaseAttrInfoService {
    ...
    public void add(BaseAttrInfo baseAttrInfo) {
        ...
        if (baseAttrInfo == null) {
            throw new mallException("参数不能为空", ResultCodeEnum.FAIL.getCode());
        }
        // 新增
        int count = baseAttrInfoMapper.insert(baseAttrInfo);
        if (count <= 0) {
            throw new mallException("新增失败，请重试！", ResultCodeEnum.FAIL.getCode());
        }
    }
    ...
    public void update(BaseAttrInfo baseAttrInfo) {
        ...
        if (count < 0) {
            throw new mallException("修改失败，请重试！", ResultCodeEnum.FAIL.getCode());
        }
    ...
    public void delete(Long id) {
        ...
        if (count < 0) {
            throw new mallException("删除失败，请重试！", ResultCodeEnum.FAIL.getCode());
        }
    }
